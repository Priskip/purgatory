...
function init(entity_id)
    ...
    SetRandomSeed(turret.x + GameGetFrameNum(), turret.y - GameGetFrameNum())
    local num = Random(1, 5)
    local image_path = "mods/purgatory/files/buildings_gfx/roboroom/mecha_turret_broken_v" .. tostring(num) .. ".xml"
    ComponentSetValue2(sprite_comp, "image_file", image_path)

    EntitySetComponentIsEnabled(turret.id, sprite_comp, true)
end
