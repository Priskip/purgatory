...
local storage_slots = getEntityInRadiusWithName(initializer.x, initializer.y, 240, "temple_alchemy_potion_storage", "temple_alchemy_station")
...
if #storage_slots ~= 0 then
    ...
    for i, slot_id in ipairs(storage_slots) do
        ...
        variableStorageSetValue(slot_id, "STRING", "mode", "place")
        ...
        --Get the Slot Number
        local slot_number = variableStorageGetValue(slot_id, "INT", "slot_number")
        ...
        if slot_contents ~= "empty" then
            ...
            local slot_contents = splitStringOnCharIntoTable(slot_contents, ";")
            ...
            header = splitStringOnCharIntoTable(header, ",")
            ...
            variableStorageSetValue(slot_id, "STRING", "mode", "pick_up")
