...
function shot( pid )
	...
	if ( comp ~= nil ) and ( comp2 ~= nil ) and ( comp3 ~= nil ) then
		...
		if ( mode == 0 ) then
			state = (state + 1) % 5
			if ( state == 1 ) then
				ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/meteor.xml" )
				ComponentSetValue2( comp3, "attack_ranged_frames_between", 220 )
			elseif ( state == 2 ) then
				ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/laser.xml" )
				ComponentSetValue2( comp3, "attack_ranged_frames_between", 170 )
			elseif ( state == 3 ) then
				ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/summon.xml" )
				ComponentSetValue2( comp3, "attack_ranged_frames_between", 300 )
			elseif ( state == 4 ) then
				if ( #help > 0 ) then
					ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/statusburst.xml" )
				else
					ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/laser.xml" )
				end
				ComponentSetValue2( comp3, "attack_ranged_frames_between", 190 )
			elseif ( state == 0 ) then
				ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/debuff_init.xml" )
		...
		elseif ( mode == 1 ) then
			...
			elseif ( state == 0 ) then
				...
				ComponentSetValue2( comp3, "attack_ranged_frames_between", 240 )
			elseif ( state == 4 ) then
				ComponentSetValue2( comp3, "attack_ranged_entity_file", "mods/purgatory/files/entities/parallel_bosses/wizard/bloodtentacle.xml" )
