...
if (#chests > 0) then
	...
	if (#players > 0) then
		...
		for i, chest_id in ipairs(chests) do
			...
			local UI_Info_Comp = EntityGetFirstComponentIncludingDisabled(chest_id, "UIInfoComponent")
			local chest_name = ComponentGetValue(UI_Info_Comp, "name")

			local distance = math.abs(x - cx) + math.abs(y - cy)

			if (distance < 48) then
				if chest_name == "$item_chest_treasure" then
					--regular chest rain
				...
				elseif chest_name == "$item_chest_treasure_super" then
					if (GlobalsGetValue("MISC_GREAT_CHEST_SAC") ~= "1") then
						--great chest sac event
						perk_spawn(cx, cy, "ROLL_AGAIN")

						collected = true
						EntityKill(chest_id)

						if collected then
							GlobalsSetValue("MISC_GREAT_CHEST_SAC", "1")
							GamePrintImportant("$log_altar_magic", "")
							AddFlagPersistent("purgatory_great_chest_sac")
						end
					end
				end
			end
