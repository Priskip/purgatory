...
function spawn_apparition(x, y)
	...
	local level = 0 -- to do: fetch biome level somehow
...
function parallel_check( x, y )
	if ( y < 0 ) then
		...
		if ( pw ~= 0 ) then
			...
			local parallel_bosses_list = {
				"mods/purgatory/files/entities/parallel_bosses/alchemist/boss_alchemist_parallel.xml",
				"mods/purgatory/files/entities/parallel_bosses/pit/bos_pit_parallel.xml",
				"mods/purgatory/files/entities/parallel_bosses/dragon/boss_dragon_parallel.xml",
				"mods/purgatory/files/entities/parallel_bosses/robot/boss_robot_parallel.xml",
				"mods/purgatory/files/entities/parallel_bosses/wizard/boss_wizard_parallel.xml"
			}

			if ( r >= 96 ) then
				-- print( "SHADOW BOSS AT " .. tostring( x + rx ) .. ", " .. tostring( y ) )
				local boss_to_spawn = parallel_bosses_list[Random(1, #parallel_bosses_list)]
				EntityLoad( boss_to_spawn, x + rx, y )
			end
		end
	end
end
