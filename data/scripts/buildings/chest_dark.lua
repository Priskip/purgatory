...
if (#keys > 0) then
	...
	if (status == 2) then
		if (already_done == false) then
			...
			CreateItemActionEntity("ALL_PROPANE", x - 56, y)
			...
			CreateItemActionEntity("ALL_DUCKS", x + 56, y)
			AddFlagPersistent("card_unlocked_alchemy")
		else
			GamePrintImportant("$log_alchemist_chest_opened", "$logdesc_alchemist_chest_opened")
			local opts = {"ALL_ACID", "ALL_NUKES", "ALL_DISCS", "ALL_ROCKETS", "ALL_BLACKHOLES", "ALL_DEATHCROSSES", "ALL_PROPANE", "ALL_DUCKS"}
