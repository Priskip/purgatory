...
dofile_once("mods/purgatory/files/scripts/perks/perk_spawn_purgatory.lua")
...
function spawn_all_shopitems(x, y)
	...
	local overstock_scrolls = EntityGetInRadiusWithTag(plyr_x, plyr_y, 24, "overstock_scroll")
	local overstock = false

	for i, eid in ipairs(overstock_scrolls) do
		local root_entity = EntityGetRootEntity(eid)
		if root_entity == player_id then
			overstock = true
		end
	end
	...
	if overstock then
		...
		--TODO: make this
		--overstocks the shop
		count = math.floor(count * 1.5 + 0.5)
		item_width = width / count
		sale_item_i = Random(1, count)
...
function spawn_all_perks(x, y)
	...
	purgatory_spawn_temple_perks(x + 30, y)
end
